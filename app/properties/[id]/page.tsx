        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8 flex flex-col md:flex-row md:justify-between md:items-end gap-6">
            <div>
              ...
              <h1 className="font-serif text-5xl md:text-6xl text-cream mb-4">{property.title}</h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
            ...
            <div className="lg:col-span-2 space-y-12">
              ...
              {/* Overview */}
              <section>
                <h2 className="font-serif text-3xl text-cream mb-6 border-b border-[#2d5441]/40 pb-4">Property Overview</h2>
              ...
              {/* Key Features Grid */}
              <section>
                <h2 className="font-serif text-3xl text-cream mb-6 border-b border-[#2d5441]/40 pb-4">Key Features</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                  <FeatureCard label="Area (Bigha)" value={property.area_bigha?.toString() || 'N/A'} icon="📏" />
                  <FeatureCard label="Area (Sq.Ft)" value={property.area_sqft?.toLocaleString() || 'N/A'} icon="📐" />
                  <FeatureCard label="Road Access" value={property.road_access || 'N/A'} icon="🛣️" />
                  <FeatureCard label="Water Connection" value={property.water ? 'Available' : 'No'} icon="💧" />
                  <FeatureCard label="Electricity" value={property.electricity ? 'Available' : 'No'} icon="⚡" />
                  <FeatureCard label="Status" value={property.status.charAt(0).toUpperCase() + property.status.slice(1)} icon="🏷️" />
                </div>
              </section>

              {/* Map Area */}
              <section>
                <h2 className="font-serif text-3xl text-cream mb-6 border-b border-[#2d5441]/40 pb-4">Location</h2>
                <div className="w-full h-[400px] bg-[#161e1a] border border-[#2d5441]/40 flex flex-col items-center justify-center p-6 text-center">
                  ...
                  <h3 className="font-serif text-2xl text-cream mb-2">Map View Limited</h3>
                  <p className="font-sans text-mist">
                    Exact coordinates for properties are shared during scheduled site visits to protect seller privacy.
                  </p>
            ...
            <div className="relative">
              <div className="sticky top-32">
                <EnquiryForm propertyTitle={property.title} propertyId={property.id} />
              </div>
            </div>
          </div>
        </div>
...
function FeatureCard({ label, value, icon }: { label: string; value: string; icon: string }) {
  return (
    <div className="bg-[#161e1a] border border-[#2d5441]/20 p-4 shrink-0 flex items-start gap-4">
      <div className="text-2xl mt-1">{icon}</div>
      <div>
        <div className="font-sans text-xs text-mist uppercase tracking-widest mb-1">{label}</div>
